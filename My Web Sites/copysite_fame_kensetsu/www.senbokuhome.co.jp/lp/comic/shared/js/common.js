...
window.onerror = function (message, url, line, column, error) {
	...
	if ($("html").hasClass("fc-debugger")) {
		...
		if (typeof url !== "undefined") {
			var fname = url.replace("\\", "../../index.html"); // correct slashes
			if (fname.lastIndexOf("../../index.html")) fname = fname.substr(fname.lastIndexOf("../../index.html") + 1);
				...
				var title = $("title",this).text(), //ã‚¿ã‚¤ãƒˆãƒ«
					...
					if(option.Image !== "no") {
						...
						if (option.noImage === "yes") {
							img = '<span style="display:block; background: url(' %2b%20src%20%2b.html') no-repeat center center; background-size:cover; width:' + option.ImageSize.width + 'px; height:' + option.ImageSize.height + 'px;"></span>';
						}

						if (option.noImage === "no") {
							if ($(html).find("img")[0]) img = '<span style="display:block; background: url(' %2b%20src%20%2b.html') no-repeat center center; background-size:cover; width:' + option.ImageSize.width + 'px; height:' + option.ImageSize.height + 'px;"></span>';
...
var checkDomain = function(url) {
	...
	return url.toLowerCase().replace(/([a-z])?:\/\//, "$1").split("../../index.html")[0];
};
